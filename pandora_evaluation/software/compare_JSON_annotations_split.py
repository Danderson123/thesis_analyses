...
def get_options():
    ...
    parser.add_argument('--chromosome-reads', dest='chromosome_reads', required=True)
    parser.add_argument('--plasmid-reads', dest='plasmid_reads', required=True)
...
def align_genes(file1, file2, output_stats, chromosome_reads, plasmid_reads):
    with open(chromosome_reads) as i:
        chromosome = set(i.read().split("\n"))
    with open(plasmid_reads) as i:
        plasmid = set(i.read().split("\n"))
    data1 = load_json(file1)
    data2 = load_json(file2)
    common_reads = set(data1.keys()) & set(data2.keys())
    all_alignments = {}
    ...
    return all_alignments, chromosome_alignments, plasmid_alignments
...
def gather_alignment_stats(alignment, output_stats):
    ...
    for read in tqdm(alignment):
        for index, col in enumerate(alignment[read]):
            ...
            if col[0] != "*" and col[1] != "*":
                ...
                # if not at read ends
                if index != 0 and index != len(alignment[read]) - 1:
                    if col[0] != "*" and col[1] == "*":
                        deletion_error += 1
                    ...
                    if col[0] == "*" and col[1] != "*":
                        ...
                        if "group_227" == col[1][1:]:
                            group_227.add(read)
                        if "group_228" == col[1][1:]:
                            group_228.add(read)
                        if "group_225" == col[1][1:]:
                            group_225.add(read)
                        if "group_1881" == col[1][1:]:
                            group_1881.add(read)
                        if "insO2" == col[1][1:]:
                            insO2.add(read)
                # if at read end
    ...
    # with open("pandora_assessment_with_AMR/0.8_0/ad_hoc/group_227_fp_reads.txt", "w") as o:
    #     o.write("\n".join(list(group_227)))
    # with open("pandora_assessment_with_AMR/0.8_0/ad_hoc/group_228_fp_reads.txt", "w") as o:
    #     o.write("\n".join(list(group_228)))
    # with open("pandora_assessment_with_AMR/0.8_0/ad_hoc/group_225_fp_reads.txt", "w") as o:
    #     o.write("\n".join(list(group_225)))
    # with open("pandora_assessment_with_AMR/0.8_0/ad_hoc/group_1881_fp_reads.txt", "w") as o:
    #     o.write("\n".join(list(group_1881)))
    # with open("pandora_assessment_with_AMR/0.8_0/ad_hoc/insO2_fp_reads.txt", "w") as o:
    #     o.write("\n".join(list(insO2)))
...
if __name__=="__main__":
    ...
    alignment, chromosome_alignment, plasmid_alignment = align_genes(args.truth_annotation, args.pandora_annotation, args.output_stats, args.chromosome_reads, args.plasmid_reads)
    ...
    # gather the alignment stats
    c_match_count, c_snp_error, c_strand_error, c_insertion_error, c_deletion_error, c_insertion_at_end, c_deletion_at_end, c_total_count, c_gene_call_counts, c_gene_DEL_counts, c_gene_SNP_counts, c_gene_IN_counts, c_pandora_called_counts = gather_alignment_stats(chromosome_alignment, args.output_stats)
    # gather the alignment stats
    p_match_count, p_snp_error, p_strand_error, p_insertion_error, p_deletion_error, p_insertion_at_end, p_deletion_at_end, p_total_count, p_gene_call_counts, p_gene_DEL_counts, p_gene_SNP_counts, p_gene_IN_counts, p_pandora_called_counts = gather_alignment_stats(plasmid_alignment, args.output_stats)
    ...
    lines = [
        ...
        f"Chromosome per gene recall: {c_match_count / (c_match_count + c_snp_error + c_deletion_error + c_strand_error)}",
        f"Chromosome per gene precision: {c_match_count / (c_match_count + c_snp_error + c_insertion_error + c_strand_error)}",
        f"Plasmid per gene recall: {p_match_count / (p_match_count + p_snp_error + p_deletion_error + p_strand_error)}",
        f"Plasmid per gene precision: {p_match_count / (p_match_count + p_snp_error + p_insertion_error + p_strand_error)}",
        ]
